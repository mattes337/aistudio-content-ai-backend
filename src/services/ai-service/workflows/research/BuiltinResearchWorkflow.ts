 ...
 *
 * Implements research using local AI models and knowledge base tools.
 * This is the default research implementation using Gemini Pro.
 */
...
/**
 * Build the retrieval strategy system prompt
 */
...
/**
 * Built-in research workflow using local AI models
 */
export class BuiltinResearchWorkflow implements ResearchWorkflow {
  readonly type = 'research' as const;
  readonly id = 'builtin';
  readonly name = 'Built-in Research';
  readonly description = 'Research using local AI models and knowledge base tools';
  ...
  async execute(request: AgentQuery): Promise<AgentResponse> {
    logger.info(`[BuiltinResearch] Query: "${request.query.substring(0, 50)}..."`);
    ...
    try {
      return await withErrorHandling(
        ...
        'BuiltinResearchWorkflow.execute'
      );
    } finally {
      clearRequestModelConfig();
      clearRequestNotebookId();
  ...
  async *executeStream(request: ResearchStreamOptions): AsyncGenerator<ResearchStreamChunk> {
    logger.info(`[BuiltinResearch] Stream: "${request.query.substring(0, 50)}..." verbose=${request.verbose} searchWeb=${request.searchWeb}`);
    ...
    try {
      ...
      // When searchWeb is enabled, search knowledge base first
      ...
      if (request.searchWeb) {
        ...
        try {
          if (!searchKnowledgeTool.execute) {
            throw new Error('searchKnowledgeTool.execute is not defined');
          }
      ...
      const generatePromise = generateText({
        ...
        tools,
        stopWhen: stepCountIs(request.maxSteps || 10),
        onStepFinish: ({ toolCalls: stepToolCalls, text }) => {
          logger.info(`Step finished: tools=${stepToolCalls?.length || 0}, textLen=${text?.length || 0}`);
      ...
      const result = finalResult as { text: string; steps: any[] } | null;
      yield {
        type: 'done',
        response: result?.text || '',
        sources: sources.length > 0 ? sources : undefined,
